...
public class MenuManager {
    ...
    private QuitGameMenu quitGameMenu;
    private ParametresPauseMenu parametresPauseMenu;
    private AudioPauseMenu audioPauseMenu;
    private ControlesPauseMenu controlesPauseMenu;
    ...
    public void changeMenu(Menu.TypeMenu typeMenu) {
        ...
        switch (typeMenu) {
            ...
            case GAME -> {
                gameView = new GameView(controller); // recrée à chaque fois
                actualMenu = gameView;
            }
            case PAUSE -> {
                if (pauseMenu == null) { pauseMenu = new PauseMenu(controller); }
                actualMenu = pauseMenu;
            }
            case QUIT_GAME -> {
                if (quitGameMenu == null) { quitGameMenu = new QuitGameMenu(controller); }
                actualMenu = quitGameMenu;
            }
            case PARAMETRES_PAUSE -> {
                if (parametresPauseMenu == null) { parametresPauseMenu = new ParametresPauseMenu(controller); }
                actualMenu = parametresPauseMenu;
            }
            case AUDIO_PAUSE -> {
                if (audioPauseMenu == null) { audioPauseMenu = new AudioPauseMenu(controller); }
                actualMenu = audioPauseMenu;
            }
            case CONTROLES_PAUSE -> {
                if (controlesPauseMenu == null) { controlesPauseMenu = new ControlesPauseMenu(controller); }
                actualMenu = controlesPauseMenu;
            }
        }
        if (actualMenu != null) {
    ...
    public void dispose() {
        ...
        if (quitGameMenu != null) quitGameMenu.dispose();
        if (parametresPauseMenu != null) parametresPauseMenu.dispose();
        if (audioPauseMenu != null) audioPauseMenu.dispose();
        if (controlesPauseMenu != null) controlesPauseMenu.dispose();
    }
}
