...
public abstract class Menu {
    ...
    public enum TypeMenu {
        SPLASH, MAIN, SELECTION, QUIT, JOUER,
        PARAMETRES, AUDIO, CONTROLES,
        COLLECTION, GAME, PAUSE, QUIT_GAME,
        PARAMETRES_PAUSE, AUDIO_PAUSE, CONTROLES_PAUSE
    }
    ...
    protected static Music ostGame;
    protected static Sound victorySound;
    protected static Sound defeatSound;
    ...
    public Menu(TypeMenu typeMenu, MenuController controller) {
        ...
        if (defeatSound == null)
            defeatSound = Gdx.audio.newSound(Gdx.files.internal("audio/defeat.mp3"));
        if (ostGame == null) {
            ostGame = Gdx.audio.newMusic(Gdx.files.internal("audio/ost_game.mp3"));
            ostGame.setLooping(true);
            ostGame.setVolume(volumeMaster * volumeMusique);
        }
        if (ostMain == null) {
    ...
    // Joue l'OST du jeu et arrête celle du menu
    public static void playGameOst() {
        if (ostMain != null && ostMain.isPlaying()) ostMain.stop();
        if (ostGame != null && !ostGame.isPlaying()) {
            ostGame.setVolume(volumeMaster * volumeMusique);
            ostGame.play();
        }
    }

    // Joue l'OST du menu et arrête celle du jeu
    public static void playMenuOst() {
        if (ostGame != null && ostGame.isPlaying()) ostGame.stop();
        if (ostMain != null && !ostMain.isPlaying()) {
            ostMain.setVolume(volumeMaster * volumeMusique);
            ostMain.play();
        }
    }
    ...
    protected static void updateVolumes() {
        ...
        if (ostGame != null) ostGame.setVolume(volumeMaster * volumeMusique);
    }
    ...
    public void buildBoardZones() {
        float margeGauche  = 54f;
        float margeDroite  = 43f;
